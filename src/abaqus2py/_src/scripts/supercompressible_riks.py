...
def main(dict):  # = 'lin_buckle'):
    ...
    slave = modelAssembly.instances[
        longerons_name].surfaces['ALL_LONGERONS_SURF']
    ...
    #    thickness=OFF)
    model.SurfaceToSurfaceContactStd(
        name='IMP_TARG', adjustMethod=NONE, clearanceRegion=None,
        createStepName='Initial',
        datumAxis=None, initialClearance=OMIT,
        interactionProperty=contact.name, main=master,
        secondary=slave, sliding=FINITE, thickness=OFF)
